...
local_rib_information_summary_t * the_loc_rib_information_summary = NULL;
...
routing_state_t * _routing_state_create(state_t * state)
{
  ...
  routing_state->the_common_loc_rib = NULL;
  
  return routing_state;
}

routing_state_t *_routing_state_create_from_ST_TR(state_t * state, routing_state_t * from_routing_state, net_addr_t node_receiving_msg)
{
  routing_state_t * routing_state;
  routing_state= (routing_state_t *) MALLOC(sizeof(routing_state_t));
  ...
  for(i=0 ; i< state->graph->tracer->nb_nodes ; i++)
  {
      if(state->graph->tracer->nodes[i]-> rid  == node_receiving_msg)
      {
         routing_state->couples_node_routing_info[i] = _couple_node_routinginfo_create(state->graph->tracer->nodes[i]);
      }
      else
          routing_state->couples_node_routing_info[i] = from_routing_state->couples_node_routing_info[i];
  }
  routing_state->the_common_loc_rib = NULL;
...
int _routing_info_compare(int nb_nodes, routing_info_t * ri1, routing_info_t * ri2)
{
    ...
    // compare node_rt_t !!!
    // normalement ne pas comparer car si rib compare ok, alors ceci ok aussi
    // car c'est dérivé de locla rib.
    // mais il faut le sauvegarder pour pouvoir l'injecter dans le système 
...
void routing_state_export_loc_rib_dot(routing_state_t * rs)
{
  ...
  if(rs->the_common_loc_rib->exported == FALSE)
  {
      //printf("on exporte dot le state %u qui a la loc rib common %u\n",rs->state->id,rs->the_common_loc_rib->id);
  //char file_name[256];
  rs->the_common_loc_rib->filename = (char*) MALLOC ( 256 * sizeof(char));
  
  sprintf(rs->the_common_loc_rib->filename,"%s%s_LOC_RIB_Common_%u.dot",rs->state->graph->tracer->base_output_directory,rs->state->graph->tracer->base_output_file_name,rs->the_common_loc_rib->id);
  //printf("FILE name : %s\n",rs->the_common_loc_rib->filename);

  gds_stream_t * stream = stream_create_file(rs->the_common_loc_rib->filename);
 ...
 stream_printf(stream, "  sep=\"+20,20\"\n");
  ...
  for( i = 0 ; i < rs->state->graph->tracer->nb_nodes ; i++)
  {
    node=rs->the_common_loc_rib->nodes_locribs[i]->node;
    //node= rs->couples_node_routing_info[i]->node;
    stream_printf(stream, "  \"");
    node_dump_id(stream, node);
    stream_printf(stream, "\" [");
  ...
  for( i = 0 ; i < rs->state->graph->tracer->nb_nodes ; i++)
  {
    ...
    bgp_sessions_info_t * bgp_sesss_info  = rs->couples_node_routing_info[i]->routing_info->bgp_sessions_info;
    //rinfo->bgp_sessions_info;
    unsigned int u;
    for( u = 0 ; u < bgp_sesss_info->nb_bgp_session_info_ ; u++)
    {
        bgp_session_info_t * one_sess_info = bgp_sesss_info->bgp_session_info[u];
        ...
        bgp_route_t * one_route;
        //for(l = 0 ; l < rinfo->bgp_router_loc_rib_t->nb_local_rib_elem ; l++)
        for(l = 0 ; l < rs->the_common_loc_rib->nodes_locribs[i]->nb_local_rib_elem ; l++)
        {
            //one_route = rinfo->bgp_router_loc_rib_t->bgp_route_[l];
            one_route = rs->the_common_loc_rib->nodes_locribs[i]->bgp_route_[l];
            if( one_sess_info->neighbor_addr == one_route->attr->next_hop )
            {
                // ajouter la flèche
                if(invert == 1)
...
*/
    ...
    //system(commande);
    sprintf(commande,"neato -T%s %s -o%s.%s",rs->state->graph->tracer->IMAGE_FORMAT,rs->the_common_loc_rib->filename, rs->the_common_loc_rib->filename,rs->state->graph->tracer->IMAGE_FORMAT);
    system(commande);
  }
  else
  {
...
#define MAX_LOCRIB_STATES 1000
#define MAX_ROUTING_STATING_HAVING_SAME_LOCRIB 100
...
// doivent être dans le même ordre  (mais normalement une seule route, donc devrait etre ok
int compare_bgp_routes(bgp_route_t ** routes1, unsigned int taille1, bgp_route_t ** routes2, unsigned int taille2)
{
    if (taille1 != taille2 )
        return DIFFERENT;
    int i =0;
    for(i=0; i< taille1 ; i++)
    {
       if ( route_equals(routes1[i],routes2[i] ) !=1 )
            return DIFFERENT;
    }
    return EQUAL;    
}

nodes_locrib_t ** extract_nodes_loc_rib(routing_state_t * rs)
{
    nodes_locrib_t  ** nodes_locribs = ( nodes_locrib_t **) MALLOC( rs->state->graph->tracer->nb_nodes * sizeof(nodes_locrib_t *));
    int i =0;
    for(i = 0 ; i < rs->state->graph->tracer->nb_nodes ; i++)
    {
        nodes_locribs[i] = ( nodes_locrib_t *) MALLOC( sizeof(nodes_locrib_t));
        nodes_locribs[i]->node = rs->state->graph->tracer->nodes[i];
        nodes_locribs[i]->nb_local_rib_elem = rs->couples_node_routing_info[i]->routing_info->bgp_router_loc_rib_t->nb_local_rib_elem;
        nodes_locribs[i]->bgp_route_ = rs->couples_node_routing_info[i]->routing_info->bgp_router_loc_rib_t->bgp_route_;
    }
    return nodes_locribs;
}

void add_state_to_local_rib_information_summary(routing_state_t * rs)
{
    int i = 0;
    if(the_loc_rib_information_summary == NULL)
    {
        the_loc_rib_information_summary = (local_rib_information_summary_t * ) MALLOC (sizeof(local_rib_information_summary_t));
    
        the_loc_rib_information_summary->nb_loc_rib_by_states = 0;
        the_loc_rib_information_summary->list_of_loc_rib_by_states = (loc_rib_States_t **) MALLOC( MAX_LOCRIB_STATES * sizeof(loc_rib_States_t *) );
        for (i=0;i<MAX_LOCRIB_STATES;i++)
           the_loc_rib_information_summary->list_of_loc_rib_by_states[i]=NULL;
    }
    
    // chercher si on a une loc rib states déjà présente :
    
    int found = FALSE;
    i=0;
    while( i < the_loc_rib_information_summary->nb_loc_rib_by_states && found == FALSE )
    {
        // regarder si on a la meme loc rib que celle-ci :
        // cad chaque noeud a la meme locrib
        int always_possible = TRUE;
        int j = 0;
        while(j < rs->state->graph->tracer->nb_nodes && always_possible == TRUE )
        { 
            // ca concerne le meme noeud car créé dans le meme ordre
            assert( rs->state->graph->tracer->nodes[j] == rs->couples_node_routing_info[j]->node );
            
            assert( the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->nodes_locribs[j]->node
                    == rs->state->graph->tracer->nodes[j] );
            
            if( compare_bgp_routes(rs->couples_node_routing_info[j]->routing_info->bgp_router_loc_rib_t->bgp_route_,
                 rs->couples_node_routing_info[j]->routing_info->bgp_router_loc_rib_t->nb_local_rib_elem ,
                 the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->nodes_locribs[j]->bgp_route_,
                 the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->nodes_locribs[j]->nb_local_rib_elem)
                == DIFFERENT )
            {
    ...
    if(found == TRUE)
    {
        // alors en position i-1
        assert(the_loc_rib_information_summary->list_of_loc_rib_by_states[i-1]->nb_routing_states > 0);
        
        the_loc_rib_information_summary->list_of_loc_rib_by_states[i-1]->routing_states[
                the_loc_rib_information_summary->list_of_loc_rib_by_states[i-1]->nb_routing_states] = 
                rs;
        
        the_loc_rib_information_summary->list_of_loc_rib_by_states[i-1]->nb_routing_states ++;           
        
        rs->the_common_loc_rib = the_loc_rib_information_summary->list_of_loc_rib_by_states[i-1];        
    ...
    else
    {
        assert(found == FALSE);
        printf(" i = %d\n", i);
        printf(" the_loc_rib_information_summary->nb_loc_rib_by_states = %u\n", the_loc_rib_information_summary->nb_loc_rib_by_states);
        assert(i == the_loc_rib_information_summary->nb_loc_rib_by_states);
        assert(the_loc_rib_information_summary->nb_loc_rib_by_states < MAX_LOCRIB_STATES);
        
       the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states] =
               (loc_rib_States_t *) MALLOC (sizeof(loc_rib_States_t));
       ...
       the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states]
               ->routing_states = (routing_state_t **) MALLOC(MAX_ROUTING_STATING_HAVING_SAME_LOCRIB * sizeof(routing_state_t *));
              
       the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states]
               ->routing_states[0] = rs;  
       
       the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states]
               ->id = the_loc_rib_information_summary->nb_loc_rib_by_states;
       
       the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states]
               ->nodes_locribs = extract_nodes_loc_rib(rs);
       
       
       
       rs->the_common_loc_rib = 
           the_loc_rib_information_summary->list_of_loc_rib_by_states[the_loc_rib_information_summary->nb_loc_rib_by_states];
