...
void graph_export_condensed_dot_to_stream(gds_stream_t * stream, graph_t * graph)
{
  ...
  stream_printf(stream, " *\n");
  //stream_printf(stream, " * Render with: neato -Tps <file.dot> <file.ps>\n");
  stream_printf(stream, " * Render with: dot <file.dot> -Teps -o<file.eps>\n");
  stream_printf(stream, " */\n");
  stream_printf(stream, "digraph Tracer_C_BGP {\n");
  stream_printf(stream, "rankdir=LR;\n\n");
  ...
  //stream_printf(stream, "  overlap=scale\n");
    ...
    //each subgraph (les loc-rib identiques)
    unsigned int i;
    unsigned int j;
    for (i = 0 ; i < the_loc_rib_information_summary->nb_loc_rib_by_states ; i++)
    {
        //the_loc_rib_information_summary->list_of_loc_rib_by_states[i];
        
        stream_printf(stream, "\tsubgraph cluster_%u {\n",i);
        stream_printf(stream, "\t\tbgcolor=\"#eeeeff\";\n");
        //stream_printf(stream, "\t\tnode [style=filled,color=gray];\n");
        
        
        sprintf(imagefilename,"%s%s_LOC_RIB_Common_%u.dot.%s",graph->tracer->base_output_directory,graph->tracer->base_output_file_name, 
                 the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->id,
                 graph->tracer->IMAGE_FORMAT);
        
        sprintf(commande,"if [ -f \"%s\" ];then exit 0; else exit 1; fi;",imagefilename);
        retour = system(commande);
        affichage = 1;
      
        if(retour == 0  && affichage !=0 )
        {
           stream_printf(stream, "\t\tlabel=<<TABLE BORDER=\"0\">\n\t\t\t\t<TR><TD>\n\t\t\t\t\t<IMG SRC=\"%s\"/>\n\t\t\t\t</TD></TR>\n\t\t\t</TABLE>>;\n\n",imagefilename);
        }
        
        // énoncer tous les états qui ont cette loc-rib.
        
        for(j=0 ; j < the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->nb_routing_states ; j++)
        {
            stream_printf(stream, "\t\t%u [\n",the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->id);
            stream_printf(stream, "\t\t\tlabel=<<TABLE BORDER=\"0\" CELLSPACING=\"0\" CELLPADDING=\"0\">\n");
            //stream_printf(stream, "\t\t\t\t<TR><TD><B>STATE %u</B><BR/>Profondeur : %u</TD></TR>\n",
            //        the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->id ,the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->depth);
            stream_printf(stream, "\t\t\t\t<TR><TD><B>STATE %u</B> -- <i>Depth : %u</i></TD></TR>\n",
                    the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->id ,the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->depth);
            
            if(fifo_tunable_depth(the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->queue_state->events) > 0)
            {
                stream_printf(stream, "\t\t\t\t<TR><TD>%u msg</TD></TR>\n",fifo_tunable_depth(the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->queue_state->events) );
                stream_printf(stream, "\t\t\t\t<TR><TD>\n");
                _queue_state_flat_simple_HTML_dump(stream,the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->queue_state);
                stream_printf(stream, "\t\t\t\t</TD></TR>\n");
            }
            stream_printf(stream, "\t\t\t\t</TABLE>>,\n");
            
            // le style de l'état :
            if( the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->type & STATE_ROOT  )
                stream_printf(stream, "%s,", STATE_ROOT_DOT_STYLE);
            if(  the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->type & STATE_FINAL  )
                stream_printf(stream, "%s,", STATE_FINAL_DOT_STYLE);
            else if(  the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->type & STATE_CAN_LEAD_TO_A_FINAL_STATE  )
                 stream_printf(stream, "%s,",  STATE_CAN_LEAD_TO_A_FINAL_STATE_DOT_STYLE);
            if( the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->nb_allowed_output_transitions != 
                    the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->nb_output)
                 stream_printf(stream, "%s,",  STATE_NOT_COMPLETELY_TREATED_DOT_STYLE);
                    
            stream_printf(stream, "\t\t   ];\n\n");
        }        
       
        stream_printf(stream, "\t}   //fin du cluster_%u\n\n",i);
  ...
  stream_printf(stream, "} //fin du digraph\n");

  
}

void graph_export_condensed_cluster_dot_to_stream(gds_stream_t * stream, graph_t * graph)
{
  gds_enum_t * nodes, * subnets;
  net_node_t * node;
  net_subnet_t * subnet;
  ...
  //stream_printf(stream, "  overlap=scale\n");
    ...
    //each loc-rib-state, les citer, ensuite on donnera les liens.
    unsigned int i;
    unsigned int j;
    unsigned int k;
    for (i = 0 ; i < the_loc_rib_information_summary->nb_loc_rib_by_states ; i++)
    {
        ...
        // si final state : mettre en couleur !!
    ...
    for (i = 0 ; i < the_loc_rib_information_summary->nb_loc_rib_by_states ; i++)
    {
        ...
        //on va parcourir et noter tous les liens de sorties !
        int voisins [the_loc_rib_information_summary->nb_loc_rib_by_states];
        for(j=0; j < the_loc_rib_information_summary->nb_loc_rib_by_states ; j++)
            voisins[j]= FALSE;
        
        for( j = 0 ; j < the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->nb_routing_states ; j++)
        {
            for(k=0; k< the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->nb_output; k++)
            {
                voisins[the_loc_rib_information_summary->list_of_loc_rib_by_states[i]->routing_states[j]->state->output_transitions[k]->to->routing_state->the_common_loc_rib->id] = TRUE;
            }            
        }
        
        for(j=0; j < the_loc_rib_information_summary->nb_loc_rib_by_states ; j++)
        {
            if( voisins[j] == TRUE && i != j)
            {
...
int graph_export_condensed_cluster_dot_to_file(graph_t * graph)
{
    ...
    sprintf(file_name,"%s%s_graph_condensed_cluster.dot",graph->tracer->base_output_directory,graph->tracer->base_output_file_name);
    gds_stream_t * stream = stream_create_file(file_name);
    graph_export_condensed_cluster_dot_to_stream(stream,graph);
...
int graph_export_condensed_dot_to_file(graph_t * graph)
{
    ...
    sprintf(file_name,"%s%s_graph_condensed.dot",graph->tracer->base_output_directory,graph->tracer->base_output_file_name);
    gds_stream_t * stream = stream_create_file(file_name);
    graph_export_condensed_dot_to_stream(stream,graph);
    stream_destroy(&stream);
    sprintf(commande,"dot -T%s %s -o%s.%s",graph->tracer->IMAGE_FORMAT,file_name, file_name,graph->tracer->IMAGE_FORMAT);
    //sprintf(commande,"dot -Tps %s -o%s.ps",file_name, file_name);
    system(commande);
