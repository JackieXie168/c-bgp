...
// @date 14/06/2005
// $Id: subnet.c,v 1.19 2008-04-11 11:03:07 bqu Exp $
// ==================================================================
...
static int _net_subnet_link_compare(void * item1, void * item2,
				    unsigned int uEltSize)
{
  net_iface_t * link1= *((net_iface_t **) item1);
  net_iface_t * link2= *((net_iface_t **) item2);

  if (link1->tIfaceAddr > link2->tIfaceAddr)
    return 1;
  if (link1->tIfaceAddr < link2->tIfaceAddr)
    return -1;
  return 0;
			     ...
			     net_subnet_type_t type)
{
  ...
  assert(len < 32); // subnets with a mask of /32 are not allowed
  subnet->prefix.tNetwork= network;
  subnet->prefix.uMaskLen= len;
 ...
 *
 */
static int _subnets_compare(void * item1, void * item2,
			    unsigned int uEltSize){
...
// ----- _subnets_destroy -------------------------------------------
static void _subnets_destroy(void * item)
{
...
{
  ptr_array_destroy((SPtrArray **) subnets_ref);
}
