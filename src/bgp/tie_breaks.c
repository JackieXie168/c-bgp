...
{
  int iHash1= path_hash(pRoute1->pASPathRef);
  int iHash2= path_hash(pRoute2->pASPathRef);
  ...
  assert(path_length(pRoute1->pASPathRef) == path_length(pRoute2->pASPathRef));
  ...
  while ((iIndex1 < path_num_segments(pRoute1->pASPathRef)) &&
	 (iIndex2 < path_num_segments(pRoute2->pASPathRef))) {
    if (pSegment1 == NULL)
      pSegment1= (SPathSegment *) pRoute1->pASPathRef->data[iIndex1];
    if (pSegment2 == NULL)
      pSegment2= (SPathSegment *) pRoute2->pASPathRef->data[iIndex2];
    if (iSegIndex1 >= pSegment1->uLength) {
      iIndex1++;
      if (iIndex1 >= path_num_segments(pRoute1->pASPathRef))
	break;
      pSegment1= pRoute1->pASPathRef->data[iIndex1];
      iSegIndex1= 0;
    }
    if (iSegIndex2 >= pSegment2->uLength) {
      iIndex2++;
      if (iIndex2 >= path_num_segments(pRoute2->pASPathRef))
	break;
      pSegment2= pRoute2->pASPathRef->data[iIndex2];
      iSegIndex2= 0;
    }
...
{
  if ((path_length(pRoute1->pASPathRef) > 1) &&
      (path_length(pRoute2->pASPathRef) > 1)) {
