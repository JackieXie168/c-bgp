...
sub node_file_create_template($$)
  {
    ...
    print NODE_FILE "  <name>$node->{name}</name>\n";
    print NODE_FILE "  <id>$node->{full_name}</id>\n";
    print NODE_FILE "  <context>$node->{context}</context>\n";

    if (scalar(@{$node->{params}}) > 0) {
      print NODE_FILE "  <parameters>\n";
      foreach (@{$node->{params}}) {
	if (m/^\[(.+)\]$/) {
	  print NODE_FILE "    <option>\n";
	  print NODE_FILE "      <name>$1</name>\n";
	  print NODE_FILE "      <description></description>\n";
	  print NODE_FILE "    </option>\n";
	} elsif (m/^\[(.+)\]\?\((.+)\)$/) {
	  print NODE_FILE "    <vararg>\n";
	  print NODE_FILE "      <name>$1</name>\n";
	  print NODE_FILE "      <cardinality>$2</cardinality>\n";
	  print NODE_FILE "      <description></description>\n";
	  print NODE_FILE "    </vararg>\n";
	} elsif (m/^\<(.+)\>$/) {
	  print NODE_FILE "    <parameter>\n";
	  print NODE_FILE "      <name>$1</name>\n";
	  print NODE_FILE "      <description></description>\n";
	  print NODE_FILE "    </parameter>\n";
	} else {
    ...
    print NODE_FILE "  <abstract></abstract>\n";
    print NODE_FILE "  <description></description>\n";
    ...
    if (exists($node->{version})) {
      print NODE_FILE "  <version></version>\n";
    }

    if (exists($node->{bugs})) {
      print NODE_FILE "  <!--<bugs>\n";
      print NODE_FILE "    <bug>\n";
      print NODE_FILE "      <id></id>\n";
      print NODE_FILE "      <date></date>\n";
      print NODE_FILE "      <status></status>\n";
      print NODE_FILE "      <description></description>\n";
      print NODE_FILE "    </bug>\n";
      print NODE_FILE "  </bugs>!-->\n";
    }
...
#
# -------------------------------------------------------------------
show_info("Parse C-BGP's list of commands");
...
foreach (sort keys %$nodes) {
  ...
  my $result= `$cmd`;
  if (length($result > 0)) {
    show_warning(node_to_filename($node)." parameters outdated");
  }
